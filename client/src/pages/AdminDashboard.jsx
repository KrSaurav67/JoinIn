        ...
        try {
            if (updateEventId) {
                const payload = {
                    ...
                    availableSeats: Number(formData.totalSeats) - bookedSeats
                };

                await api.put(`/events/${updateEventId}`, payload); // update existing
                // Event.findByIdAndUpdate(req.params.updateEventId, {availableSeats : bookedSeats}, {new : true});
    ...
    return (
        <div className="max-w-7xl mx-auto">
            <div className="bg-black text-white rounded-2xl p-6 sm:p-8 mb-8 shadow-lg flex flex-col md:flex-row justify-between items-center gap-6 text-center md:text-left">
                ...
                >
                    {showEventForm ? (updateEventId ? 'Cancel Update' : 'Cancel Creation') : '+ Create New Event'}
                </button>
            </div>
            ...
            {showEventForm && (
                <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 mb-8 animation-slideDown">
                    <h2 className="text-2xl font-bold mb-6 text-gray-800">{updateEventId ? "Update Event" : "Create New Event"}</h2>
                    <form onSubmit={updateEventId ? handleUpdateEvent : handleCreateEvent} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <button type="submit" className="md:col-span-2 bg-gray-900 text-white font-bold py-3 mt-2 rounded-lg hover:bg-black transition shadow-md">{updateEventId ? "Update" : "Publish Event"}</button>
                    </form>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="flex flex-col">
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                        <h2 className="text-2 font-bold mb-6 text-gray-800 flex items-center gap-3 mt-3 underline">
                        <span className="flex items-center justify-center w-8 h-8 rounded-full bg-gray-100 text-gray-600 text-sm">{events.filter(event => new Date(event.date) >= new Date()).length}</span>
                            Upcoming Events
                        </h2>
                        <ul className="divide-y divide-gray-100 max-h-[600px] overflow-y-auto">
                            {events.filter(event => new Date(event.date) >= new Date()).length === 0 ? <li className="p-6 text-gray-500 text-center">No events created yet.</li> :
                                events.filter(event => new Date(event.date) >= new Date()).map(event => (
                                    <li key={event._id} className="p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:bg-gray-50 transition border-b border-gray-100 last:border-0">
                                        ...
                                        <div>
                                            <button onClick={() => openUpdateForm(event)} className="w-full sm:w-auto text-green-500 hover:text-white hover:bg-green-500 border border-green-200 px-4 py-2 rounded-lg text-sm font-bold transition shadow-sm shrink-0 mr-3">
                                                Update
                                            </button>
                            ...
                            <h1 className="text-2 font-bold mb-6 text-gray-800 flex items-center gap-3 mt-5.5 underline">
                            <span className="flex items-center justify-center w-8 h-8 rounded-full bg-gray-100 text-gray-600 text-sm">{events.filter(event => new Date(event.date) < new Date()).length}</span>
                                Expired Events
                            </h1>
                            {events.filter(event => new Date(event.date) < new Date()).length === 0 ? <li className="p-6 text-gray-500 text-center">No events created yet.</li> :
